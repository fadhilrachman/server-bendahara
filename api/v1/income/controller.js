...
const createData = async (req, res, next) => {
  const { tanggal, total_pemasukan, kategori, deskripsi } = req.body;
  try {
...
const getOne = async (req, res, next) => {
  const { id } = req.params;
  try {
    const result = await income.findOne({ where: { id }, attributes });
...
const update = async (req, res, next) => {
  ...
  try {
    const result = await income.update(
      { tanggal: formatedDate, total_pemasukan, kategori, deskripsi },
      { where: { id } }
    );
    !result
      ? res.status(404).json("data tidak ditemukan")
